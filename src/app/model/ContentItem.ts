...
import {
  durationDefaultContentItem,
  fechaCreacionDefault,
  FechaCreacionSinceDefault,
  FechaCreacionUntilDefault,
  IContentItemRating,
  IContentType,
  maxDurationUntil,
  maxDurationVideo,
  maxFechaCreacion,
  maxFechaCreacionSince,
  maxRatingFilter,
  minDurationSince,
  minDurationVideo,
  minFechaCreacion,
  minFechaCreacionSince,
  minRatingFilter,
  RatingDefault,
} from './Interfaces';
...
export class ContentItem {
  ...
  constructor(
    ...
    title: string = '',
    description: string = '',
    contentType: IContentType = IContentType.Web,
    tags: Array<string> = [],
    duration: Duration = durationDefaultContentItem,
    rating: IContentItemRating = RatingDefault,
  ...
  containDescriptionOrTitle(titleOrDescription: string): Boolean {
    if (this._title.toLowerCase().includes(titleOrDescription.toLowerCase())) {
      ...
      this._description.toLowerCase().includes(titleOrDescription.toLowerCase())
    ) {
      return true;
    } else {
      return false;
  ...
  addTag(tag: string) {
    ...
    customLogger.logDebug(
      `Se ha agregado ${tag}. El nuevo tagArray es:${this._tags}`
    );
  }
  ...
  removeTag(tag: string) {
    if (this.containTags([tag])) {
      ...
      customLogger.logDebug(
        `Se ha removido ${tag}. El nuevo tagArray es:${this._tags}`
      );
    } else {
      throw new ErrorExternoAlPasarParams(
        `No existe la etiqueta ${tag} que se quiso eliminar.`
      );
    }
  }
  ...
  set duration(duration: Duration) {
    if (duration > maxDurationVideo || duration < minDurationVideo) {
      throw new ErrorExternoAlPasarParams(
        `La duracion debe estar entre ${maxDurationVideo} y ${minDurationVideo} `
      );
    }
    this._duration = duration;
  }
  ...
  containsItemsBetweenTwoDurations(
    durationSince: Duration = minDurationSince,
    durationUntil: Duration = maxDurationUntil
  ): Boolean {
    // Since > Until, throw Error.
    if (durationUntil !== undefined && durationSince > durationUntil) {
      throw new ErrorExternoAlPasarParams(
        `Since: ${durationSince} , no puede ser menor a Until: ${durationUntil} .`
      );
    }
    ...
    let thisDurationInMinutes;
    // Todo revisar. deberia estar en duration.
    if (typeof this._duration === 'number') {
      thisDurationInMinutes = this._duration; // Si ya esta en minutos, no pasar.
    } else {
      thisDurationInMinutes = this._duration.getDurationTotalInMinutes(
        this._duration
      );
    }

    // let thisDurationInMinutes = this._duration.getDurationTotalInMinutes(
    //   this._duration
    // );
    let durationSinceMinutes =
      durationSince.getDurationTotalInMinutes(durationSince);
    let durationUntilMinutes =
      durationSince.getDurationTotalInMinutes(durationUntil);

    customLogger.logDebug(
      `Desde containsItemBetweenTwoDuration: thisDurationMinutes:${durationSinceMinutes}, durationSince ${durationSinceMinutes} y durationUntil ${durationUntilMinutes}`
    );

    // Ambos parametros existen.
    if (durationUntil !== undefined) {
      if (
        durationSinceMinutes <= thisDurationInMinutes &&
        thisDurationInMinutes <= durationUntilMinutes
      ) {
        return true;
      } else {
        return false;
   ...
   */
  containsRating(
    ratingSince: IContentItemRating = minRatingFilter,
    ratingUntil: IContentItemRating = maxRatingFilter
  ): Boolean {
    customLogger.logDebug(
      `Desde containsRating(), this._rating=${this._rating}, ratingSince=${ratingSince} y ratingUntil=${ratingUntil}`
    );
  ...
  set fechaCreacion(fechaCreacion: Date) {
    if (fechaCreacion > maxFechaCreacion || fechaCreacion < minFechaCreacion) {
      throw new ErrorExternoAlPasarParams(
        `La fechaCreacion debe estar entre ${maxFechaCreacion} y ${minFechaCreacion} `
      );
    }
    this._fechaCreacion = fechaCreacion;
  }
  ...
  containsFechaCreacion(
    fechaCreacionSince: Date = FechaCreacionSinceDefault,
    ...
    if (typeof this._fechaCreacion === 'string') {
      // si fechaCreacion es una cadena, la convertimos en un objeto de fecha
      let date = new Date(Date.parse(this._fechaCreacion));
      fechaCreacionFormateada = date.getTime();
    } else {
      // si fechaCreacion ya es un objeto de fecha, simplemente obtenemos la marca de tiempo
      fechaCreacionFormateada = this._fechaCreacion.getTime();
    }

    customLogger.logDebug(
      `Desde containsFechaCreacion(), this._fechaCreacion=${this._fechaCreacion}, fechaCreacionSince=${fechaCreacionSince} y fechaCreacionUntil=${fechaCreacionUntil}`
    );

    if (
      fechaCreacionSinceFormateada > maxFechaCreacionSince.getTime() ||
      fechaCreacionSinceFormateada < minFechaCreacionSince.getTime()
    ) {
      throw new ErrorExternoAlPasarParams(
        `AAAA La fechaCreacionSince debe estar entre ${minFechaCreacionSince} y ${maxFechaCreacionSince}. Actual: ${fechaCreacionSince}, ${fechaCreacionUntil} `
      );
    }
  ...
  toString(): string {
    return `ContentItem: Title=${this._title}, ContentType=${
      this._contentType
    }, Duration=${JSON.stringify(this._duration)}, fechaCreacion=${
      this._fechaCreacion
    }, rating=${this._rating}, tags=${this._tags}, descripcion=${
      this._description
    }`;
  }
}
