  ...
  onViewVideo(videoUrl: string) {
     try {
      ...
      const dialogRef = this._dialog.open(VideoPlayComponent, {
        ...
        this._clientNotification.openNotification(`${error}`)      
     }
  }
  ...
  */
  onCreateContent() {
    ...
    dialogRef.afterClosed().subscribe(response => {
      ...
      if (response) {
        this._customLogger.logDebug("onCreateContent()", "Cerrar, respuesta form: true")
        return this.applyFilterGeneral()
      }
      this._customLogger.logDebug("onCreateContent()", "Cerrar, respuesta form: false")
    })
  }
  ...
  onDeleteContent(id: number)  {
    ...
    dialogRef.afterClosed().subscribe(response => {
      if (response) {
        ...
        this._clientNotification.openNotification("Eliminado con exito!")
      } else {
        this._customLogger.logDebug("ContentItemList, [onDeleteContent]", "dialog response =>", response)
      }
    })
