...
import { IOrderArray, maxLimitByPage, minLimitByPage } from "src/app/model/Interfaces";
import { IContentManagerService } from "src/app/services/interface/IContentManagerService";
// import { IContentManagerServiceMock } from "src/app/services/interface/IContentManagerServiceMock";
import { ContentManagerServiceMock } from "src/app/services/mock/content-manager-service-mock.service";
import { CustomLogger } from "../CustomLogger";
import { ErrorNoExisteLaPagina } from "../pagination/error/ErrorNoExisteLaPagina";
...
export class PageIterator {
    ...
    private _service: IContentManagerService; // Todo Deberia ser IContentManagerService, no el mock...
     ...
     */
    constructor(service: IContentManagerService = new ContentManagerServiceMock(new CustomLogger()), filter: ContentItemFilter = new ContentItemFilter(), limit: number, order: IOrderArray = IOrderArray.ASC) {   
    ...
    private isValidPage(page: number): boolean {
        if (page <= 0) {
            ...
            throw new ErrorNoExisteLaPagina(`La pagina no puede ser mayor a la ultima. Page:${page} Paginas totales: ${this._totalPages}`)
        }
        return true; 
    }
